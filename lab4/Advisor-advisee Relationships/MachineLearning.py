...
# model_svm,_ = SVM(X_train,y_train,X_test,y_test)
model_dt,_ = DTree(X_train,y_train,X_test,y_test)
...
def output_prediction():
    ...
    print_res(predicted_arr_num,'result/mentor_unilateral_result_tmp.csv')
    print("Outputing finished!")  
    
    file_input_author = open('coauthor_feature_raw.csv','r')
    file_input_res = open('result/mentor_unilateral_result_tmp.csv','r')
    file_output = open('result/MentorshipPredictionResult_IDPair2Probability.csv','w')
